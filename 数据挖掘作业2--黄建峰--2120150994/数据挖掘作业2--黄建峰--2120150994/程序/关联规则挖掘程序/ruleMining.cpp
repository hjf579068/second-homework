...
int main()
{
	...
	/*第一次扫描，计算每个候选项的频数，并将频数在最小支持度之上的项记录下来*/
	...
	/*将单项项目拼接产生包含两项的候选项，并计算候选项的频数，然后将频数在最小支持度之上的项记录下来*/
		...
		for(int j = 0; j < freCount[i];j++)
		{
			if(freItem[i][j][i-1] == 8 ||freItem[i][j][i-1] == 9){
				...
				if(conf>=0.8){
					...
					printf("->%d",freItem[i][j][k]);
					printf(":   ");
					printf("%.f÷120 = %.3f;   ",countTemp[0], supp);
					printf("%.f÷%.f = %.3f",countTemp[0], countTemp[1], conf);
					if(freItem[i][j][i - 1]== 8)
					{
						printf(";   %.3f÷0.492=%.3f", conf, conf/0.492);
					}
					if(freItem[i][j][i - 1]== 9)
					{
						printf(";   %.3f÷0.417=%.3f", conf, conf/0.417);
					}
					printf("\n");
				}
			}
